...
export class ScoreService {
  ...
  getPosts(){
    this.http.get<{message: string, scores: any }>("http://localhost:3000/api/getscore")
    .pipe(map((scoreData) => {
  ...
  addScore(name: string, cpm: number, title: string, date: Date){
    ...
    this.http.post<{message: string, scoreId: string}>('http://localhost:3000/api/savescore', score)
